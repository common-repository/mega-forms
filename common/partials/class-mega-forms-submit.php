...
class MF_Form_Submit
{
	...
	public function validate_submitted_form()
	{
		...
		$submit_referrer = mfget_cleaned_url(mfpost('_mf_referrer', $this->posted));
		$submit_nonce    = mfpost('_mf_nonce', $this->posted);

		$session_token_id    = get_mf_session_token_id($this->form->ID, $submit_referrer);
		$session_referrer_id = get_mf_session_referrer_id($this->form->ID, $submit_referrer);

		$session_referrer = mfget_cleaned_url(mf_session()->get($session_referrer_id));
		$session_nonce    = mf_session()->get($session_token_id);

		$bypass_session_errors = apply_filters('mf_bypass_session_error', false, $submit_referrer, $session_referrer, $submit_nonce, $session_nonce);

		if (!$bypass_session_errors) {
			// Check if submit nonce exists
			if (empty($submit_nonce)) {
				throw new Exception($this->get_validation_text('form_validation_session_error') . ' (Error Code: MF001)');
				// Error Code MF001: Submit nonce is missing
			}

			// Check if session nonce exists
			if (empty($session_nonce)) {
				throw new Exception($this->get_validation_text('form_validation_session_error') . ' (Error Code: MF002)');
				// Error Code MF002: Session nonce is missing
			}

			// Check if submit referrer exists
			if (empty($submit_referrer)) {
				throw new Exception($this->get_validation_text('form_validation_session_error') . ' (Error Code: MF003)');
				// Error Code MF003: Submit referrer is missing
			}

			// Check if session referrer exists
			if (empty($session_referrer)) {
				throw new Exception($this->get_validation_text('form_validation_session_error') . ' (Error Code: MF004)');
				// Error Code MF004: Session referrer is missing
			}

			// Compare nonces
			if ($submit_nonce !== $session_nonce) {
				throw new Exception($this->get_validation_text('form_validation_session_error') . ' (Error Code: MF005)');
				// Error Code MF005: Nonce mismatch
			}

			// Compare referrers
			if ($submit_referrer !== $session_referrer) {
				throw new Exception($this->get_validation_text('form_validation_session_error') . ' (Error Code: MF006)');
				// Error Code MF006: Referrer mismatch
			}
		}

		// Validate wp nonce
		$wp_nonce        = mfpost('_mf_extra_nonce', $this->posted, false);
		$verify_wp_nonce = !empty($wp_nonce) ? wp_verify_nonce($wp_nonce, 'mf_form_' . $this->form->ID) : false;

		if (!$verify_wp_nonce && !$bypass_session_errors) {
			if (empty($wp_nonce)) {
				throw new Exception($this->get_validation_text('form_validation_wpnonce_error') . ' (Error Code: MF007)');
				// Error Code MF007: WordPress nonce is missing
			} else {
				throw new Exception($this->get_validation_text('form_validation_wpnonce_error') . ' (Error Code: MF008)');
				// Error Code MF008: WordPress nonce verification failed
			}
		}
	...
	public function validate_entry_form()
	{
		...
		$entry_nonce = mfpost('mf_entry_' . $this->form->ID . '_token', $this->posted, false);
		if (!wp_verify_nonce($entry_nonce, 'mf_save_entry_' . $this->form->ID)) {
	...
	public function validate_fields($fields, $validate_required_fields = true)
	{
		...
		foreach ($fields as $field) {
			...
			// ignore non-submitting hidden fields
			if ($fieldObject->type == 'hidden' && mfget_bool_value($fieldObject->get_setting_value('field_non_submitting'))) {
				continue;
			}
			...
			// then we do not need to do any additional validation ( except if it's a `file` input or an unlisted field )
			if (empty($fieldValue) && empty(mfget($fieldObject->get_field_key(), $_FILES)) && !$fieldObject->isUnlistedField) {
				continue;
			}
			...
			if ($validation !== true) {
				...
				if (isset($validation['notice']) || isset($validation['compound_notices'])) {
					...
					if (isset($validation['notice'])) {
						...
						$notice      = isset($validation['notice']) ? $validation['notice'] : '';
			...
			// If the field is an unlisted field, and the sanitized value is empty, we don't need to process it any further.
			if ($fieldObject->isUnlistedField && empty($sanitizedValue)) {
				continue;
			}
