...
class MF_Field
{
	 ...
	 * Or, set manually in the case of unlisted fields ( where `$this->isUnlistedField` is `true` )
	 *
	 * @var int
	 */
	public $field_id = 0;
	...
	/**
	 * Should we hide the field on the backend
	 * If set to `true`, the field needs to be added to the form manually including any related handling.
	 * Not adding the field to the form dynamically will prevent certain processing tasks related to form submission.
	 *
	 * @var bool
	 */
	public $isUnlistedField = false;
		...
		if (!empty($custom_settings)) {
			foreach ($custom_settings as $optionObj) {
				...
				if (is_array($option_display_logic)) {
					...
					} elseif ($option_display_logic['exclude']) {
						$display = true;
		...
		foreach ($settings as $sKey => $sVal) {
			...
			if (!empty($settings[$sKey])) {
				foreach ($settings[$sKey] as $key => $val) {
					...
					} elseif (!is_numeric($key) && is_array($val) && (!isset($settings[$sKey][$key]['callback']) || !isset($settings[$sKey][$key]['priority']))) {
						// If an array is provided but it's missing the main keys, add them
						if (!isset($settings[$sKey][$key]['callback'])) {
							$settings[$sKey][$key]['callback'] = $key;
						}
						if (!isset($settings[$sKey][$key]['priority'])) {
							$settings[$sKey][$key]['priority'] = 99;
						}
					} elseif (!isset($settings[$sKey][$key]['callback']) || !isset($settings[$sKey][$key]['priority'])) {
	...
	public function get_the_field($value = '', $is_editor = false)
	{
		...
		// If we are in the editor view, return an empty string if the field is unlisted.
		// This will prevent the field from being displayed in the form builder even if it was added to the form fields manually using filter hooks.
		if ($this->isUnlistedField && $this->is_editor) {
			return '';
		}
	...
	public function get_field_settings()
	{
		...
		// Return an empty string if the field is unlisted.
		// This will prevent the field from being displayed in the form builder even if it was added to the form fields manually using filter hooks.
		if ($this->isUnlistedField) {
			return '';
		}
	...
	public function add_button_to_container($fields_container)
	{
		...
		// Do not add the field button to any container if it is unlisted.
		// This will ensure the field is not available in the form editor view.
		if ($this->isUnlistedField) {
