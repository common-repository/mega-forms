...
if (!function_exists('mfget_form')) :
	...
	function mfget_form($form_id = 0)
	{
		...
		return apply_filters('mfget_form', $_form);
	}
			...
			foreach ($form->containers['data'] as $container) {
				...
				foreach ($container['columns'] as $col) {
					...
					foreach ($col['fields'] as $id) {
						$ids[] = $id; // Field ids are usually an integer, but we allow for custom string with unlisted fields.
					}
				}
			}
		}

		// Precompute the array of ids to their positions to avoid repeated linear searches
		$positions = array_flip($ids);
		// Use this information to sort the form fields based on the ids' order
		uksort(
			$form->fields,
			function ($key1, $key2) use ($positions) {
				// If one of the keys does not exist in the position mapping, fallback to a default comparison
				if (!isset($positions[$key1])) {
					return 1;
				}
				if (!isset($positions[$key2])) {
					return -1;
				}

				// Subtract the positions to get ordering: < 0 if $key1 should come before $key2, > 0 otherwise
...
if (!function_exists('mfupdate_option')) :
	...
	function mfupdate_option($key, $value, $type = '')
	{
		...
		// Update the option
		$updated = update_option('megaforms_' . $key, $value);
...
if (!function_exists('mfinput')) :
	...
	function mfinput($type, $args = array(), $is_editor = false)
	{
		...
		*/
		...
		if ($is_editor) {
			if (isset($args['onchange_edit'])) {
				$attrs['data-edit'] = $args['onchange_edit'];
			}
			if (isset($args['onchange_preview'])) {
				$attrs['data-preview'] = $args['onchange_preview'];
			}
		}
				...
				<li id="<?php echo $tab; ?>_settings" class="mfsetting_container <?php echo $classes; ?>">
					...
					foreach ($options as $name => $args) {
						...
						if ($mf_inputs) {
							...
							if (isset($args['value']) && ($args['type'] !== 'radio' && $args['type'] !== 'checkbox')) {
								$attrs['value'] = esc_attr($args['value']);
							}
							...
							if (isset($args['parent'])) {
								...
								$attrs['data-conditional-rules'] = json_encode(
									array(
										'container' => '.sub-setting',
										'rules'     => array(
											'name'     => $args['parent'],
							...
							?>
											<tr>
												...
												<td class='mf-inner-field mf-field-type-<?php echo $args['type']; ?>'>
													<?php
													...
													switch ($args['type']) {
														case 'switch':
															if (mfget_bool_value($args['value'])) {
...
if (!function_exists('mf_clear_all_external_cache')) :

	function mf_clear_all_external_cache()
	{
		// Clear page cache for popular caching plugins
		...
		// Special logic for Varnish
		$purge_urls = array(home_url('/'), get_site_url());
