...
class Mega_Forms_Updater
{
	...
	public static function update_db($current_db_version = '')
	{
		...
		/**
		 * Add a background task to attempt clearing all cache that is external to the plugin (eg; WP Object Cache, Varnish, WPRocket..etc )
		 *
		 */
		mf_tasks()->push_to_queue(
			array(
				'type' => 'callback',
				'data' => 'mf_clear_all_external_cache',
			)
		)->save()->dispatch();
