...
class MF_Container_Page extends MF_Container
{
	...
	public function get_container_settings()
	{
		...
		/**
		 * Since `page_names` field is dynamic, we need to generated custom HTML for it
		 *
		 */
		$pages = mfget(
            'page_names',
		...
		return array(
			'tabs'    => array(
				...
				'behavior' => __('Behavior', 'megaforms'),
			),
			'options' => array(
				'general'  => array(
				...
				'behavior' => array(
					'enable_auto_advance' => array(
						'type'         => 'radio',
						'label'        => __('Enable Auto Advance', 'megaforms'),
						'desc'         => __('Automatically proceed to the next step/page once all required fields are completed. Note that this will only works if AJAX is enabled.', 'megaforms'),
						'value'        => mfget('enable_auto_advance', $this->settings, 'no'),
						'options'      => array(
					...
					'auto_advance_mode'   => array(
						'type'         => 'radio',
						'label'        => __('Auto Advance Mode', 'megaforms'),
						'desc'         => __('Choose how the auto-advance feature should be applied: to all pages or to specific pages only.', 'megaforms'),
						'value'        => mfget('auto_advance_mode', $this->settings, 'all_pages'),
						'options'      => array(
							'all_pages'      => __('All Pages', 'megaforms'),
							'specific_pages' => __('Specific Pages', 'megaforms'),
						),
						'sanitization' => 'string',
						'parent'       => 'enable_auto_advance',
						'parent_value' => 'yes',
					),
					'auto_advance_pages'  => array(
						'type'         => 'text',
						'label'        => __('Auto Advance Pages', 'megaforms'),
						'desc'         => __('Enter the page numbers separated by commas where auto-advance should be applied. E.g., 1, 3, 5', 'megaforms'),
						'placeholder'  => '1,3,6',
						'value'        => mfget('auto_advance_pages', $this->settings),
						'sanitization' => 'string',
						'parent'       => 'auto_advance_mode',
						'parent_value' => 'specific_pages',
					),
				),
			),
		);
	...
	public function get_page_break()
	{
		...
		if ($this->is_editor) {
			...
			if ($current_page > 1) {
				$footer_args['before_submit'] = get_mf_button(
                    'button',
                    $prev_text,
                    array(
