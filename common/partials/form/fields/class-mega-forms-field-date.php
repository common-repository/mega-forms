...
class MegaForms_Date extends MF_Field
{
	...
	public function is_valid_date($date, $format, $is_range = false)
	{
		...
		if ($is_range) {
			$date       = htmlentities($date, ENT_NOQUOTES, 'utf-8');
			$date       = str_replace('&nbsp;', '', $date);
			$date_parts = explode('-', $date);
			$is_valid   = true;
			foreach ($date_parts as $date_part) {
				$d        = DateTime::createFromFormat($php_format, trim($date_part));
				$is_valid = $d && $d->format($php_format) == trim($date_part);
			...
			$d = DateTime::createFromFormat($php_format, trim($date));
			return $d && $d->format($php_format) == trim($date);
		}
	}
