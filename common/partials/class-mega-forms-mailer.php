...
class MF_Mailer
{
    ...
    public static function send($emails, $subject, $message, $args = array())
    {
        ...
        /**
         * Allows changes to final email parameters.
         *
         */

        $mail_data = apply_filters('mf_mail_data', compact('recipients', 'subject', 'content', 'headers', 'attachments'));

        // Send the email
        return wp_mail($mail_data['recipients'], $mail_data['subject'], $mail_data['content'], $mail_data['headers'], $mail_data['attachments']);
    }
