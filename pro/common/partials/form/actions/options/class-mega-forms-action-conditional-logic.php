...
class MegaForms_Action_Conditional_Logic extends MF_Action_Option
{
	...
	public function get_option_display($action)
	{
		...
		if ($rules_count === 0) {
			$cl_inputs .= $this->get_rules_group_markup(
                array(
					1 => array(
						'field'    => '',
						'operator' => 'is',
	...
	public function get_rules_group_markup($rules_group, $index, $action, $is_enabled)
	{
		...
		if (is_array($rules_group) && !empty($rules_group)) {
			...
			foreach ($rules_group as $key => $rule) {
				...
				// Value
				$rulesHTML .= '<td>';
				$rulesHTML .= '<input type="text"' . mf_esc_attrs(
                    array(
	...
	public function get_rule_operators_markup($namekey, $selected, $is_enabled)
	{
		...
		$operators     = array(
			'is'               => __('Is', 'megaforms'),
			'isnot'            => __('Is not', 'megaforms'),
			'greaterthan'      => __('Greater than', 'megaforms'),
			'lessthan'         => __('Less than', 'megaforms'),
			'contains'         => __('Contains', 'megaforms'),
			'doesnotcontain'   => __('Doesn\'t contain', 'megaforms'),
			'beginswith'       => __('Begins with', 'megaforms'),
			'doesnotbeginwith' => __('Doesn\'t begin with', 'megaforms'),
			'endswith'         => __('Ends with', 'megaforms'),
			'doesnotendwith'   => __('Doesn\'t end with', 'megaforms'),
			'isempty'          => __('Is empty', 'megaforms'),
			'isnotempty'       => __('Isn\'t empty', 'megaforms'),
		);
				...
				if (isset($rules['group'])) {
					...
					foreach ($rules['group'] as $rule) {
						...
						if ($is_group_rules_met == false && 'and' == $relation) {
							$is_rule_met = false;
							break;
						} elseif ($is_group_rules_met && 'or' == $relation) {
							$is_rule_met = true;
							break;
						}
