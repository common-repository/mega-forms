...
class MF_Action_Email extends MF_Action
{
	...
	public function pre_process_action($postedFields = array())
	{
		...
		if (strpos($from_email, '{mf:wp') !== false) {
			$from_email = $this->process_merge_tags($from_email, $postedFields);
		}
	...
	public function cc()
	{
		...
		$args['id']                = $this->get_action_field_key($key);
		$args['value']             = $this->get_setting_value($key);
		$args['conditional_rules'] = $this->get_js_conditional_rules(
            'show',
            array(
