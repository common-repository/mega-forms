...
class MegaForms_Choice extends MF_Field
{
	...
	protected function field_choices()
	{
		...
		foreach ($options as $optkey => $optval) {
			...
			$repeatable_inputs .= get_mf_input(
                'text',
                array(
					'name'      => $name . '[label]',
					'class'     => 'label',
					'value'     => $optval['label'],
					'data-edit' => $change_label,
                )
            );
			$repeatable_inputs .= get_mf_input(
                'text',
		...
		$repeatable_inputs .= get_mf_checkbox(
            array(
				'id'        => 'mf_enable_choices_values_' . $this->field_id,
				'data-edit' => $toggle_values,
            )
        );
	...
	/**
	 * Returns the field for the choice alignment.
	 *
	 * @return string
	 */
	// protected function field_alignment()
	// {

	// $label     = __('Alignment', 'megaforms');
	// $field_key = 'field_alignment';
	// $type     = $this->type == 'checkboxes' ? 'checkbox' : 'radio';
	// $value    = $this->get_setting_value($field_key);
	// $desc     = __('Add choices to this field using the repeatable inputs below.', 'megaforms');
	...
	// $args['value'] = $value;
	// $args['options'] = array(
	// 'vertical'   => __('Vertical', 'megaforms'),
	// 'horizontal' => __('Horizontal', 'megaforms'),
	// 'grid'       => __('Grid', 'megaforms'),
	// );
	// $args['default'] = 'vertical';
	// $args['onchange_preview'] = $this->get_js_helper_rules('none', 'update_choice_alignment');
	 ...
	 * @return string
	 */
	// protected function field_style()
	// {

	// $label     = __('Style', 'megaforms');
	// $field_key = 'field_style';
	// $type     = $this->type == 'checkboxes' ? 'checkbox' : 'radio';
	// $value    = $this->get_setting_value($field_key);
	// $desc     = __('Add choices to this field using the repeatable inputs below.', 'megaforms');
	...
	// $args['value'] = $value;
	// $args['options'] = array(
	// 'standard' => __('Standard', 'megaforms'),
	// 'boxed' => __('Boxed', 'megaforms'),
	// );
	// $args['default'] = 'vertical';
	// $args['onchange_preview'] = $this->get_js_helper_rules('none', 'update_choice_style');
	...
	public function sanitize_settings()
	{
		...
		foreach ($choices['choices'] as &$choice) {
			...
			if (isset($choice['label'])) {
				$choice['label'] = $this->type == 'select' ? sanitize_text_field($choice['label']) : wp_kses($choice['label'], $this->allowedHTML);
			}
