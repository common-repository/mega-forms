...
class MFAPI
{
	...
	public static function get_form($form_id, $type = false)
	{
		...
		if (false === $type) {
			...
			$form_meta = self::get_bulk_form_meta(
				$form_id,
				array(
					'field_types',
					'fields',
		...
		/**
		 * Allow the form object to be filtered as soon as it's retrieved from the database
		 *
		 */
		$_form = apply_filters('mf_get_form', $_form);

		/**
		 * Hold a reference to the form object
		 * use this global for treating form merge tags, post submission
		 *
		 * Only save to the global scope if we have all types defined.
		 * This is done to avoid conflicts and missing properties when
		 * the form object is pulled from the global scope.
		 */
		if (false === $type) {
		...
		if ('entries' == $type) {
			...
			if (!empty($fields)) {
				foreach ($fields as $field) {
					...
					if ($field_object->type == 'hidden' && mfget_bool_value($field_object->get_setting_value('field_non_submitting'))) {
						continue;
					}
	...
	public static function prepare_form_settings($settings)
	{
		...
		foreach ($options as $key => $option) {
			...
			} elseif ($option['sanitization'] == 'boolean') {
				$prepared[$key] = false;
			}
		}
	...
	public static function get_field_setting($field)
	{
		...
		$type = $field['type'] ?? '';
		// If field type is not defined, bail out
		if (!$type) {
			return '';
		}
	...
	public static function get_field($field, $is_editor = false)
	{
		...
		if ($megaform_field->isUnlistedField && $is_editor) {
			return '';
		}
	...
	public static function get_fields($form, $is_editor = false)
	{
		...
		if (!empty($form->containers) && !empty($form->containers['data'])) {
			...
			if (!$is_editor) {
				...
				$honeypot_obj   = new MegaForms_Honeypot(
					array(
						'form_id'  => $form->ID,
						'field_id' => 0,
					)
			...
			// If we have no containers, create an empty container ( row type container )
	...
	public static function get_validation_required_notice($field = null)
	{
		...
		if (!empty($field) && isset($field['type']) && ($field['type'] == 'choice' || $field['type'] == 'radios' || $field['type'] == 'select' || $field['type'] == 'checkboxes')) {
			...
			if (!$option) {
				...
				$notice = sprintf(__('%s is required.', 'megaforms'), mfget('field_label', $field, __('This field', 'megaforms')));
			} else {
				$notice = $option;
			}
		}
	...
	public static function get_ready_form_templates($get_single = false, $args = array())
	{
		...
		$templates['additional'] = apply_filters(
			'mf_ready_form_templates',
			array(
