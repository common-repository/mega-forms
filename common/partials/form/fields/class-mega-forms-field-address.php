...
class MegaForms_Address extends MF_Field
{
		...
		foreach ($addressComponents as $aKey => $aVal) {
			...
			if ($enabled || $this->is_editor) {
				...
				if ('country' == $aKey) {
					...
					$inputHTML .= get_mf_input(
                        'text',
                        array(
							'name'        => $subArgs['id'],
							'value'       => $subArgs['value'],
							'placeholder' => $subArgs['placeholder'],
                        )
                    );
				}
	...
	protected function address_inputs()
	{
		...
		foreach ($addressComponents as $aKey => $aVal) {
			...
			// Sub Label Field
			$address_inputs .= get_mf_input(
                'text',
                array(
					'name'         => sprintf('%s[desc]', $name),
					'value'        => $aVal['desc'],
					'class'        => 'desc active',
					'placeholder'  => $aVal['label'],
					'data-default' => $aVal['label'],
					'data-preview' => $update_sublabel,
                )
            );

			// Default Value Field
			if ('country' == $aKey) {
				$address_inputs .= get_mf_select(
                    array(
						'name'         => sprintf('%s[default]', $name),
						'value'        => $aVal['default'],
						'placeholder'  => '',
						'class'        => 'default',
						'data-preview' => $update_value,
                    ),
                    $this->get_countries()
                );
			} else {
			...
			if ('country' == $aKey) {
				$address_inputs .= get_mf_input(
                    'text',
                    array(
						'name'         => sprintf('%s[placeholder]', $name),
						'value'        => $aVal['placeholder'],
						'class'        => 'placeholder',
						'data-default' => $countryPlaceholder,
						'data-preview' => $update_placeholder,
                    )
                );
			} else {
				$address_inputs .= get_mf_input(
	...
	public function is_valid_zip_code($country, $value)
	{
		...
		$country_regex = apply_filters(
            'mf_zip_code_validation_regex',
            array(
				'US'      => '/\d{5}([\-]?\d{4})?/i',
