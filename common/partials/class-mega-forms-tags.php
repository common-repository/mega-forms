...
class MF_Merge_Tags
{
	...
	{
		$form                   = mfget_form();
		$from_fields            = mfget_form_fields($form);
		$display_section_titles = apply_filters('mf_display_all_fields_tag_sections', true);
		ob_start();
		?>
		<table cellspacing="0">
			<tbody>
				...
				if (!empty($from_fields)) {
					foreach ($from_fields as $field) {
						...
						// Ignore static fields, except section field
						if ($fieldObject->isStaticField && $fieldObject->type !== 'section') {
							continue;
						}
						...
						// If the field is a `section` handle it differently
						if ($fieldObject->type == 'section') {
							// If displaying section titles is disabled, move to next iteration
							if (!$display_section_titles) {
								continue;
							}
							?>
							<tr>
								<th colspan="2" valign="top" style="width: 100%; padding: 15px 10px; padding-top: 20px; text-align: left; font-weight: 600; font-size: 110%; border-bottom: 1px solid #e8e8e8;"><?php echo esc_html($label); ?></th>
							</tr>
							<?php
						} elseif (isset($fields[$fieldID])) {

							$formatted_value = $fields[$fieldID]['values']['formatted_short'];
							?>
							<tr>
								<th valign="top" style="min-width: 150px; padding: 15px 10px; text-align: left; font-weight: 600;"><?php echo esc_html($label); ?></th>
								<td valign="top" style="padding: 15px 10px; text-align: left;"><?php echo $formatted_value; ?></td>
							</tr>
