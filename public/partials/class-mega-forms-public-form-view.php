...
class MF_Form_View
{
	...
	public function form_display()
	{
		...
		$this->form_attrs = apply_filters(
			'mf_view_form_tag_attributes',
			array(
				'id'      => sprintf('mega-form-%d', absint($this->form->ID)),
				'class'   => 'single-mega-form mf-mask',
				'method'  => 'post',
		...
		if ($is_posted && $is_success && $keep_form !== true) {
			...
			$footer_args['submit_attribues'] = array(
				...
				'value'          => $this->form->settings['submit_button'],
			);
	...
	public function form_hidden_inputs()
	{
		...
		$session_id          = mf_session()->get_session_id();
		...
		$wp_nonce = wp_create_nonce('mf_form_' . $this->form->ID);
		...
		// Unique form token
		echo '<input type="hidden" name="_mf_nonce" value="' . esc_attr($form_token) . '">';
		// WP Nonce
		echo '<input type="hidden" name="_mf_extra_nonce" value="' . esc_attr($wp_nonce) . '">';
		// Timestamp
		echo '<input type="hidden" name="_mf_t_token" value="' . base64_encode(json_encode(time() * $this->form->ID)) . '">';
		// Safety/security unique token that changes on each page load ( can be used for debugging or processing that depends on unique key per submission )
